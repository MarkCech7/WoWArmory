...
def get_character_armory_url(name: str) -> str:
    ...
    if not result or result == "No results found.":
        ...
        return f"{phrase}\nPlease try to search for a character <a href=\"http://localhost:5173/armory/\" target=\"_blank\">here</a>."

    phrase = character_found_chain.invoke({"name": name})

    return f"{phrase}\nTo view {name}'s armory click <a href=\"http://localhost:5173/armory/{name}\" target=\"_blank\">here</a>."
...
def get_arena_leaderboards_url(query: str) -> str:
    ...
    return f"{phrase}\nYou can visit leaderboards here: <a href=\"http://localhost:5173/leaderboards/2v2\" target=\"_blank\">2v2 Arena Ladder</a>, <a href=\"http://localhost:5173/leaderboards/3v3\" target=\"_blank\">3v3 Arena Ladder</a>"
